...
double calcSimilar(std::vector<float> feature1, std::vector<float> feature2)
{
...
double calcSimilarEuler(std::vector<float> feature1, std::vector<float> feature2)
{
    double sim = 0.0;
//    printf("%i %i", feature1.size(), feature2.size());
    for (int i = 0; i < feature1.size(); i++)
    {
        sim += pow(feature1[i]-feature2[i], 2);
    }
    return sim;
}
